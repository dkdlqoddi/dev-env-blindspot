...
1. **Analyze.** Spawn IN PARALLEL (one message, two Agent calls): `change-analyzer` (subagent_type: `change-analyzer`) with the base ref (default: merge-base with the default branch — main, else master), the touched spec paths, the area `map.md` path (when it exists), and a plan document path when one exists (`docs/swarm/plan.md` when the work ran as an Antigravity swarm); and `check-runner` (subagent_type: `check-runner`) with the commands from `rules.md` 표준 명령 (or the project's known checks).
