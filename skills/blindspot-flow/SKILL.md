...
4. Implementation — offer (Korean) 이 세션에서 직접 구현 or 에이전트 협업 네트워크(스웜)로 병렬 구현. In-session: `work-report` notes mode opens `docs/notes/<slug>.md`; implementation proceeds. Collaborative Network: `swarm-plan` writes and reviews `docs/swarm/` (squad briefs for worker, verifier, reviewer, audited across 5 network dimensions via `swarm-plan-reviewer`); then run `/swarm-run` (squads collaborate via the `task` tool and result records); then `swarm-review` audits the result and either loops back to `swarm-plan` for a delta round or continues to stage 5
