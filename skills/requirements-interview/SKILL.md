...
2. **Ground before asking.** Spawn ONE `codebase-scanner` agent (`TypeName: codebase-scanner`) with lens `conventions` and the task description BEFORE writing questions. Include `docs/blindspot/` in its scan targets: past requirements/unknowns/report docs on adjacent topics record decisions the user already made. Questions that ignore the actual code waste the user's time. Skip only if the project has no code yet.
