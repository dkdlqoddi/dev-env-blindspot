...
3. **Interview.** In Korean, ONE question per message, via the `ask_question` tool with 2–4 concrete options where possible.
...
5. **Verify.** Spawn `doc-verifier` (`TypeName: doc-verifier`) on the spec, naming the sections you filled (요구사항, 결정 기록, 열린 질문). Fix every reported issue, re-save. Then run `python3 .antigravity/skills/work-report/scripts/docs_check.py <spec path> <map.md path, when you edited the map>` (the `work-report` skill's `scripts/docs_check.py`) and fix every violation. Do not skip on PASS-looking drafts — verification is not optional.
