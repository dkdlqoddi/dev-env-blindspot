...
3. **Fold decisions into the notes.** Ensure `docs/notes/<slug>.md` exists (the 작업 노트 line of the plan names it; create it from the `work-report` skill's `templates/notes.md`, installed at `.claude/skills/work-report/templates/notes.md`, when missing). For every 결정 line the auditor returned, append one notes entry in the notes-mode format — in Korean, although the result files are English — with 결정 주체 the task id, 계획과의 이탈 from the result's 브리프와 다르게 한 것, and 사용자 확인 필요 예 when the worker chose between options the spec does not settle. 막힌 것 lines become entries too, marked 사용자 확인 필요 예. This happens before report mode so the decisions surface in the quiz.

4. **Triage and route.** From the audit table, the check-runner result, and the plan's 회차 table, sort the residue and tell the user (Korean, one AskUserQuestion when more than one route is viable):
   ...
   - A task still not 완료 확인 although an earlier 회차 row's 범위 already named it → the swarm has failed it twice, which is a capability problem, not a brief problem. Implement it in this session under `work-report` notes mode and commit it; the next re-plan round removes it from the plan.
   - Tasks 실패, 보류, 결과 없음, or 통합 위험 spanning tasks → `swarm-plan` re-plan round: it rewrites only those briefs (folding the 막힌 것 text in) and appends a 회차 row; the user runs `/swarm-run` again, then this skill again.
...
- A third round for a task the swarm failed twice repeats the same miss and pays again for a brief rewrite and an audit in this session — past the second round, implementing it here is the cheaper path.
- A commit whose subject ends in (검증 실패) is the 웨이브 that stopped the run, committed on purpose so the audit diff shows the failure — it is evidence, not a merge candidate.
