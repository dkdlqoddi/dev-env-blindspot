...
2. **Audit.** Spawn via `task` tool: `swarm-auditor` (`TypeName: swarm-auditor`) with the plan path, `docs/swarm/tasks/`, `docs/swarm/results/`, the status path, and the 기준 커밋; and `check-runner` (`TypeName: check-runner`) with the 전체 검증 command. Do not read the result files yourself — the auditor distills them.
