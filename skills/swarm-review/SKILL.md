...
2. **Audit.** Spawn via `task` tool: `swarm-auditor` (`TypeName: swarm-auditor`, Reasoning model e.g. DeepSeek-R1 / 32B) with the plan path, `docs/swarm/tasks/`, `docs/swarm/results/`, the status path, and the 기준 커밋; and `check-runner` (`TypeName: check-runner`) with the 전체 검증 command. Do not read the result files yourself — the auditor distills them.
...
4. **Triage and route.** From the audit table and the check-runner result, sort the residue and tell the user (Korean, question tool or chat message when more than one route is viable):
