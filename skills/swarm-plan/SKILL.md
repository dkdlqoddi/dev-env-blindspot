...
2. **Decompose.** Cut the work into tasks that satisfy all of:
   ...
   - **Squad-coordinated** — each task establishes a collaborative squad: `swarm-worker` (구현), `swarm-verifier` (검증), and `swarm-reviewer` (리뷰) interacting via the `task` tool under a 3-turn convergence budget.
...
4. **Multi-Faceted Plan & Network Review.** Validate the entire collaboration package before committing:
   ...
   - **Agent network collaboration audit**: Spawn via `task` tool: `swarm-plan-reviewer` (`TypeName: swarm-plan-reviewer`) with `docs/swarm/plan.md`, `docs/swarm/tasks/`, the target spec, and area rules. It audits the package across 5 core dimensions: (1) Squad architecture & task sizing, (2) Communication and task protocol contract, (3) Deadlock prevention & 3-turn convergence, (4) Cross-task interface consistency, (5) Check command isolation & feasibility. Fix every reported issue (높음/중간) before proceeding.
...
6. **Hand off.** Commit the package (`git add docs/swarm docs/notes && git commit -m "swarm: 계획 <slug>"`), then tell the user (Korean): 에이전트 네트워크 협업 계획 및 다각도 사전 리뷰(스쿼드 분담, 프로토콜, 데드락 방지, 인터페이스 정합성, 검증 격리) 결과를 요약 제시하고 `/swarm-run` 실행을 안내합니다 — 채팅에 `/swarm-run`을 입력하거나 CLI에서
   `opencode run --command swarm-run` (또는 OpenCode 채팅창에서 `/swarm-run` 입력)
