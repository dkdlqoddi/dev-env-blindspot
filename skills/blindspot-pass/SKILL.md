...
2. **Scan.** Spawn TWO `codebase_scanner` custom agents IN PARALLEL through the available collaboration tools: lens `integration-points` and lens `edge-cases`. Each receives the task description, relevant decision rows, verification commands from `AGENTS.md`, and the paths or globs the task touches, with the instruction to report only what the rows do not already settle. Select the installed custom profile when the Codex surface supports it. If it cannot select that profile, read `.codex/agents/codebase_scanner.toml` and include its complete `developer_instructions` in each general subagent task. If subagent spawning remains unavailable, the parent performs both lenses under those same read-only instructions. These two lenses are the pass's whole exploration; do not scan the same ground again. Skip only when the project has no code.
