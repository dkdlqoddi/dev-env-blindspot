...
2. **Scan.** Spawn TWO `codebase-scanner` agents IN PARALLEL (one message, two `invoke_subagent` tool calls, `TypeName: codebase-scanner`): lens `integration-points` and lens `edge-cases`. Each receives the task description, the relevant decision rows, the verification commands from ANTIGRAVITY.md, and the paths or globs the task touches, with the instruction to report only what the decision rows do not already state. These two scanners are the pass's whole exploration — do not launch other exploration agents over the same ground. Skip only when the project has no code.
...
4. **Ask.** Ask the remaining questions in ONE ask_question call (the tool takes at most 4), in Korean, 2–4 concrete options each, plain language first and technical terms in parentheses. Skip this step when no question remains.
...
- Do not serialize the two scanner spawns — one message, two `invoke_subagent` tool calls, or the pass takes twice as long.
...
- A row that repeats what ANTIGRAVITY.md already says belongs in ANTIGRAVITY.md, not here.
