...
2. **Fan out scanners.** Spawn `codebase-scanner` agents IN PARALLEL (one message, multiple `invoke_subagent` tool calls, `TypeName: codebase-scanner`), one per lens:
   ...
   If domain territory exists, add ONE `domain-researcher` agent (`TypeName: domain-researcher`) to the same parallel batch, with: the domain topic, the task description, and what the user already knows.
