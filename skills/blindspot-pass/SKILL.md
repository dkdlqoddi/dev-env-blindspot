...
6. **Verify.** Run the custom agent `doc_verifier` on the file. Select the named custom-agent profile, not merely the same task label. If this Codex surface cannot select it, read that exact file under `.codex/agents/` and include its complete `developer_instructions` with the task input in a general subagent. Thread-cap handling is mandatory. Keep a rejected role pending. Wait for a subagent slot to become available, then retry it. Never continue to the next step or drop the role. If subagent spawning remains unavailable, run the required role in the parent using that exact profile's complete `developer_instructions` and the same task input; do not skip it. Fix every issue, re-save.
