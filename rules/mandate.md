...
2. Skills MUST delegate exploration and verification to their designated agents (`codebase-scanner`, `domain-researcher`, `doc-verifier`, `change-analyzer`, `check-runner`, `swarm-auditor`), and swarm execution to `swarm-worker` / `swarm-checker`. Never dump raw exploration output into the main context.
