...
2. Skills MUST delegate exploration and verification to their designated agents (`codebase-scanner`, `domain-researcher`, `doc-verifier`, `change-analyzer`, `check-runner`, `swarm-auditor`), and collaborative swarm execution to `swarm-worker` / `swarm-verifier` / `swarm-reviewer` / `swarm-checker`. Never dump raw exploration output into the main context.
