...
2. Skills MUST delegate exploration and verification to their designated agents (`codebase_scanner`, `domain_researcher`, `doc_verifier`, `change_analyzer`, `check_runner`). Parent execution with the complete designated profile's `developer_instructions` and the same task input is the only exception when subagent spawning remains unavailable after the skill retains rejected work, waits for a subagent slot, and retries every pending role. Never dump raw exploration output into the main context.
