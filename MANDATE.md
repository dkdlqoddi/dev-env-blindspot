...
| The user asks to hand implementation to an Antigravity swarm ("스웜으로", "병렬로 나눠서"), or blindspot-flow stage 4 takes the swarm route | `swarm-plan` |
| An Antigravity swarm run has finished (`docs/swarm/status.md` says 진행 끝남) or the user asks to review swarm results | `swarm-review` |
...
2. Skills MUST delegate exploration and verification to their designated agents (`codebase-scanner`, `domain-researcher`, `doc-verifier`, `change-analyzer`, `check-runner`, `swarm-auditor`). Never dump raw exploration output into the main context.
3. All user-facing deliverables (interview questions, documents, quizzes) are written in Korean.
4. Lifecycle skills write only the tier files and the working files above; they never create any other document under `docs/`.
