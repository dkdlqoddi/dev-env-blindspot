...
4. This workflow creates no other document under `docs/`. There is no PR merge gate: commit finished, verified work directly on the default branch (`main`) and push it — no feature branch or pull request unless the user asks, never force-push. Summaries and review points go in the commit message body.
5. `blindspot-pass` delegates exploration to `codebase-scanner`; raw scan output never lands in the main context.
