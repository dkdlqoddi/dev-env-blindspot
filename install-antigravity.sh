...
echo "blindspot-swarm: installed — .agents/{skills,agents,rules} symlinked; once docs/swarm/plan.md exists, run: agy --add-dir \"\$PWD\" -p '/swarm-run' --model gemini-3.8-flash-high --effort high --dangerously-skip-permissions --print-timeout 60m (the swarm runs only on Gemini 3.8 Flash High)"
