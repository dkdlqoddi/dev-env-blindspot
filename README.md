...
| 성격 | 어디서 | 모델 | 단계 |
|---|---|---|---|
| **판단** — 한 번, 전체 맥락으로 | Claude Code | 고성능 (Opus 5 / Fable) | 인터뷰, 사각지대 점검, 설계, 스웜 **계획**, 결과 **감사**, 보고·퀴즈 |
| **실행** — 여러 번, 맥락 없이, 병렬로 | Antigravity | 고속 (Gemini 3.8 Flash High) | 브리프대로 구현, 검증 명령 실행 |
...
agy --add-dir "$PWD" -i '/swarm-run' --model gemini-3.8-flash-high --dangerously-skip-permissions
```

`--model`이 곧 구현 모델이다. 워커 에이전트 파일이 `model: inherit`라서 dispatcher와 같은 모델로 돈다. agy 1.2.2에서 실측해 보니 에이전트 파일의 `model`이 호출 때 넘기는 값보다 우선했고, `model: flash`로 두면 `--model`과 상관없이 gemini-3.8-flash-tiered로 돌았다. checker만 `model: flash`로 싸게 돈다.
...
**Claude Code에서 `swarm-review`** — `swarm-auditor`가 세션 모델로 작업별 "완료했다"는 주장을 brief·diff와 대조해 판정표(완료 확인 / 범위 이탈 / 미완 / 검증 불일치 / 결과 없음)를 만들고, 워커의 결정을 `docs/notes/<slug>.md`로 옮긴다. 테스트를 지우거나 느슨하게 만든 diff는 검증 불일치로 잡는다. 그다음 갈래는 넷이다.
...
| 자리 | 권장 | 이유 |
|---|---|---|
| Claude Code — `swarm-plan` | Opus 5 xhigh (요구가 모호하면 Fable) | 분해 품질이 스웜 전체의 품질이다. 브리프 작성이 이 사이클에서 Claude 토큰을 가장 많이 쓴다 |
| Claude Code — `swarm-review` | Opus 5 high | 스킬은 라우팅만 한다. 판정은 세션 모델을 물려받은 `swarm-auditor`가 한다 |
| `swarm-auditor` | 세션 모델 상속 | Flash의 완료 주장을 diff와 대조하는 유일한 자리라 아끼지 않는다 |
| Antigravity dispatcher·워커 (`--model`) | `gemini-3.8-flash-high` | 워커 파일이 `model: inherit`라 `--model`이 곧 구현 모델이다. dispatcher는 스크립트 출력만 옮기므로 판단 부담이 작다. `agy models`로 목록 확인 |
| `swarm-checker` (에이전트 파일의 `model`) | `flash` | 명령 한 번 실행과 요약만 한다 |

Claude Code 스킬 frontmatter에는 model·effort 필드가 없다. 단계마다 세션의 모델·effort 설정을 바꿔서 쓴다.
...
| `swarm-auditor` | 작업별 완료 주장을 브리프·결과·diff와 대조(세션 모델 상속). 범위 이탈, 테스트 약화, 통합 위험, 워커 결정 원문 |
...
| `swarm-checker` | subagent (`model: flash`, 읽기 전용) | 검증 명령을 한 번 돌리고 실패만 30줄 이내로, 스크립트가 읽는 고정 형식으로 보고 |
...
| 워커가 High가 아닌 모델로 돎 | 워커 파일이 `model: inherit`인지, 명령에 `--model gemini-3.8-flash-high`가 있는지 확인. `model: flash`면 `--model`과 무관하게 flash-tiered로 돈다 |
...
```bash
bash test/check.sh   # main의 11개 검사 + Antigravity 파일 frontmatter·모델·도구 허용목록(2b) + install-antigravity 멱등성과 설치 경로의 swarm_next.py(5) + 스웜 템플릿 계약(8) + swarm_check git 기준 정상·음성 fixture(12) + swarm_next 상태 전이 시나리오(13, test/swarm_scenario.py)
```
...
설계 문서: `docs/superpowers/specs/2026-09-08-antigravity-swarm-design.md` (두 하네스 분업의 결정과 대안, agy 1.1.27 실측)와 후속 `docs/superpowers/specs/2026-09-14-swarm-hardening-design.md` (외부 리뷰 반영, swarm_next.py 상태 기계, agy 1.2.2 실측). 이 저장소 자체의 3계층 문서: `docs/core/`. main의 설계 이력: `docs/superpowers/specs/`, 3계층 이전 이력: `docs/blindspot/`.
