...
질문은 객관식 위주로 한 번에 하나씩 제시되며, 코드를 몰라도 답할 수 있는 한국어로 작성된다. 근거로 결정할 수 있는 내용은 스스로 해소하고 기록한다. 한 번의 조사에서 질문 후보가 일곱 개를 넘으면 먼저 추가 조사로 줄인다. 구현 중 되돌릴 수 있는 선택은 보수적인 기본값으로 진행하고 다음 확인 지점에 모아서 묻는다.
...
스레드 상한 때문에 작업이 대기하면 모든 조사 관점을 그대로 유지하고 모든 결과가 끝난 뒤 종합한다. 멀티 에이전트 기능 자체가 비활성화된 환경에서는 부모가 같은 전체 역할 지침으로 각 관점을 차례로 수행한다. 어떤 경우에도 관점, 검증 단계, 산출물 계약을 줄이지 않는다.
