...
| "이 코드베이스 처음인데 뭘 조심해야 하지?" / "내가 모르는 게 뭐지?" | `blindspot-pass` | 4개 관점(관례/유사기능/통합지점/엣지케이스)으로 병렬 스캔해서, 놓치기 쉬운 것들을 "결정 가능한 질문"으로 바꿔 확인받는다 |
...
skill/agent를 추가·제거하면 `test/check.sh`의 파일 수(`-eq 8`)·skill 목록과 `MANDATE.md` 매핑표를 함께 갱신해야 한다 (`CLAUDE.md`의 Consumer contract 체크리스트 참고).
