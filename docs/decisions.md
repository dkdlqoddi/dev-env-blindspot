...
| 2026-09-25 | core | PR 머지 게이트를 없앤다. 완료하고 검증한 작업은 PR 없이 기본 브랜치(main)에 직접 커밋·푸시하고, 요약과 리뷰 포인트는 커밋 메시지 본문에 쓴다 | 사용자 지시(소비 프로젝트 tradingview-strategy 세션). main 브랜치의 2026-09-19 행(a3ebcba)을 codex 브랜치에 이식하며 2026-09-14 행(퀴즈 게이트를 PR 리뷰로 대체)을 대체한다 | PR 리뷰 게이트 유지; 소비 프로젝트 AGENTS.md 재정의로만 처리 | 사용자 |
