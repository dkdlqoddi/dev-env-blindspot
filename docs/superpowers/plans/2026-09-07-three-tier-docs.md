...
- Produces: spec의 목적과 배경·동작 방식·의도적 범위 제외를 고쳐 쓰는 유일한 스킬. 상태를 `확정`으로 바꾸는 유일한 스킬.
