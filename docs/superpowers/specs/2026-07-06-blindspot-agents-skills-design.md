...
### 4.5 blindspot-flow (오케스트레이터)

- **트리거**: 사용자가 `/blindspot-flow` 호출 또는 "전체 플로우로 진행" 요청 시. 새 기능을 처음부터 끝까지 이 체계로 진행하고 싶을 때
- **워크플로우**: 라이프사이클 skill을 순서대로 호출하는 얇은 오케스트레이터 — 자체 로직 최소화
  1. `requirements-interview` 호출 → 요구사항 문서 확보
  2. `blindspot-pass` 호출 → unknowns 해소
  3. `explainer` 호출 → 설계 문서 확보
  4. `work-report` 노트 모드 시작 → 구현 진행 (구현 자체는 이 skill 범위 밖, 노트 기록만 강제)
  5. 구현 완료 시 `work-report` 보고 모드 → 보고서 + Pre-Merge Quiz
- 각 단계 사이에 사용자에게 진행 여부 확인 (단계 건너뛰기 허용 — 예: 이미 요구사항 문서가 있으면 1 생략)
- 이미 산출물이 존재하는 단계는 감지하고 재사용 제안
- **산출물**: 없음 (하위 skill들의 산출물이 전부)
...
- 작업유형 → 필수 skill 매핑 표:
  ...
  - 기능을 처음부터 끝까지 진행 요청 → `blindspot-flow` (전체 라이프사이클 오케스트레이션)
