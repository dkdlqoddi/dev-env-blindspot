...
| 요구사항 | requirements-interview | requirements-interview (blindspot-pass: 도메인 수용 기준) | — (다른 스킬은 열린 질문으로만 이의 제기) |
...
- 4번: "Lifecycle skills write only the tier files and the two working files above; they never create any other document under `docs/`."
...
| 최종 리뷰: "부트스트랩 전용 호출에 출구가 없어 인터뷰 전에 blindspot 질문이 나감"(§6.2 0단계 vs §8-6) | blindspot-pass 0단계에 부트스트랩 전용 출구 추가. 스펙 기원 |
| 최종 리뷰: "MANDATE hard rule 4가 docs/ 아래 모든 문서를 금지해 소비 프로젝트의 다른 문서와 충돌"(§5) | "라이프사이클 스킬은 다른 문서를 만들지 않는다"로 범위 축소. 스펙 기원 |
| 최종 리뷰: "답한 열린 질문 행을 지우는 스킬이 없고, spec 생성 시 맵의 상세 명세 칸을 뒤집는 스킬이 없음" | 세 스킬에 삭제·뒤집기 문장 추가, work-report도 spec 생성 가능 |
| 최종 리뷰: "blindspot-flow 2단계 재사용 신호가 결정 기록이라 항상 건너뛰기를 제안" | 신호를 '점검 대기 열린 질문 없음'으로 교체 |
