...
| agents | 탐색, 조사, 검증, diff 분석, 스웜 계획 검토 및 구현·검증·리뷰·감사를 맡는 서브에이전트 11종 | agents/*.md | 없음 |
...
| 스웜 사이클 | swarm-plan | swarm-handoff(swarm-plan, swarm_check.py, swarm-plan-reviewer) → swarm-handoff(swarm-run, worker/verifier/reviewer/checker) → swarm-handoff(swarm-review, swarm-auditor) → lifecycle-skills(work-report) |
